-- =========================================================================
-- STGame — Etapa 1.12, parte B1: tarefa compartilhada, tablet do balcão,
-- ficha e senha do tablet, acesso mais rápido, rodízio no aceite e a
...
--   20260928100400_ficha_do_tablet.sql
--   20260928100500_senha_do_tablet_a_mao.sql
--   20260928100600_acesso_diz_se_ha_login.sql
--   20260928100700_rodizio_no_aceite.sql
--   20260928100800_configuracao_que_falta.sql
-- =========================================================================

BEGIN;

-- -------------------------------------------------------------------------
-- Preparo: funções que MUDARAM DE FORMATO dentro deste próprio pacote.
-- O Postgres não deixa um CREATE OR REPLACE trocar o que a função devolve.
-- Numa segunda rodada, a versão nova já está lá e a primeira definição do
-- pacote quebraria. Apagar antes resolve, e elas são recriadas logo abaixo,
...
-- =========================================================================
DO $verifica$
DECLARE v_falta text[]; v_chaves text;
BEGIN
  SELECT coalesce(array_agg(f ORDER BY f), ARRAY[]::text[]) INTO v_falta
    FROM unnest(ARRAY['pegar_tarefa', 'revogar_aceite', 'atribuir_tarefa', 'fila_da_loja',
                      ...
                      'visao_fila', 'visao_pessoa_do_pin', 'visao_pegar', 'visao_entregar',
                      'fechamento_valendo', 'ficha_dos_tablets', 'registrar_evento_acesso_loja',
                      'tentativa_abrir_ex', 'marcar_senha_amao', 'erros_de_login', 'meu_acesso',
                      'rodizio_espera', 'elegiveis_da_tarefa']) f
   WHERE NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
                      WHERE n.nspname = 'public' AND p.proname = f);
  IF array_length(v_falta, 1) > 0 THEN
    RAISE EXCEPTION 'Faltou criar: %', array_to_string(v_falta, ', ');
  END IF;
  IF to_regclass('public.tarefascandidatos') IS NULL OR to_regclass('public.acessoslojaeventos') IS NULL THEN
    RAISE EXCEPTION 'Faltou criar alguma tabela desta etapa.';
  END IF;

  -- Toda chave de configuração tem de existir em TODA conta. Foi assim que a
  -- seção "Aceite de tarefas" apareceu vazia em 25/09/2026.
  SELECT string_agg(DISTINCT c.contaid || ':' || chaves.k, ', ') INTO v_chaves
    FROM public.contas c
    CROSS JOIN LATERAL (
  ...
  IF v_chaves IS NOT NULL THEN
    RAISE EXCEPTION 'Configuração não chegou em alguma conta: %', v_chaves;
  END IF;
