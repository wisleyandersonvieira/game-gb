...
-- ficha e senha do tablet, acesso mais rápido e rodízio no aceite.
--
...
BEGIN
  ...
  END IF;
  -- Toda conta que existe tem de ter a configuração do rodízio. (Num banco
  -- sem conta nenhuma, não há o que conferir.)
  IF EXISTS (SELECT 1 FROM public.contas c
              WHERE NOT EXISTS (SELECT 1 FROM public.configuracoes g
                                 WHERE g.contaid = c.contaid AND g.chave = 'MINUTOS_RODIZIO_ACEITE')) THEN
    RAISE EXCEPTION 'A configuração do rodízio não chegou em alguma conta.';
  END IF;
