...
BEGIN
  FOREACH f IN ARRAY ARRAY[
    ...
    'public.conta_do_codigo(text)',
    'public.usar_codigo_acesso(integer, text, text)',
    'public.diagnostico_do_sistema(jsonb)'
  ] LOOP
...
BEGIN
  ...
  -- ASSINATURA: conferir so o NOME nao bastava. A entrega no tablet quebrou
  -- por uma funcao que tinha mudado de parametros, e a /saude dizia "tudo
  -- certo" (25/09/2026). Agora o aplicativo manda o que espera.
  d := public.diagnostico_do_sistema(
         '{"eu_inicio": ["p_contaid", "p_funcionarioid"]}'::jsonb);
  PERFORM public.exigir(jsonb_array_length(d->'assinaturasdiferentes') = 0,
                        'assinatura certa nao e acusada');

  d := public.diagnostico_do_sistema('{"eu_inicio": ["p_outro_nome"]}'::jsonb);
  PERFORM public.exigir(jsonb_array_length(d->'assinaturasdiferentes') = 1,
                        'parametro com nome diferente e acusado');
  PERFORM public.exigir((d->'assinaturasdiferentes'->>0) LIKE '%p_contaid%',
                        'e a acusacao diz o que o banco tem de verdade');

  d := public.diagnostico_do_sistema('{"eu_inicio": ["p_contaid"]}'::jsonb);
  PERFORM public.exigir(jsonb_array_length(d->'assinaturasdiferentes') = 1,
                        'parametro a MENOS tambem e acusado');

  d := public.diagnostico_do_sistema(
         '{"eu_inicio": ["p_contaid", "p_funcionarioid", "p_extra"]}'::jsonb);
  PERFORM public.exigir(jsonb_array_length(d->'assinaturasdiferentes') = 1,
                        'parametro a MAIS tambem e acusado');

  d := public.diagnostico_do_sistema('{"funcao_que_nunca_existiu": []}'::jsonb);
  PERFORM public.exigir((d->'assinaturasdiferentes'->>0) LIKE '%nao existe no banco%',
                        'funcao que sumiu e acusada, com o motivo');

  -- Funcao que devolve TABELA: proargnames traz tambem o nome das colunas, e
  -- elas nao sao parametro de ninguem. Sem esse cuidado, fila_da_loja seria
  -- acusada para sempre.
  d := public.diagnostico_do_sistema('{"fila_da_loja": ["p_lojaid"]}'::jsonb);
  PERFORM public.exigir(jsonb_array_length(d->'assinaturasdiferentes') = 0,
                        'funcao que devolve tabela nao e acusada pelas colunas dela');
END $$;
...
-- Conta CANCELADA fecha tambem as LEITURAS do celular. Achado na revisao
-- adversarial de 25/09/2026: eu_confere_pessoa olhava so funcionarios.ativo,
-- entao o Inicio, as tarefas e o extrato abriam numa conta cancelada. A
-- escrita ja era barrada por minha_conta_editavel().
--
-- Roda no MESMO contexto da secao 55 (papel do servidor, sem uid): as eu_*
-- exigem contexto de servidor. Chamadas como 'authenticated' dariam erro de
-- qualquer jeito, e as provas passariam pelo motivo errado — foi o que
-- aconteceu na primeira versao deste bloco, e o controle "volta a abrir"
-- abaixo e justamente quem denuncia isso.
RESET ROLE;
SET teste.uid = '';
...
DECLARE deu_erro boolean;
BEGIN
  -- Controle: com a conta ativa, o Inicio abre.
  BEGIN PERFORM public.eu_inicio(1, 9803); deu_erro := false;
  EXCEPTION WHEN OTHERS THEN deu_erro := true; END;
  PERFORM public.exigir(NOT deu_erro, 'com a conta ativa, o Inicio do colaborador abre');

  UPDATE public.contas SET status = 'cancelada' WHERE contaid = 1;
  ...
  -- E volta a abrir: a trava nao gruda.
  BEGIN PERFORM public.eu_inicio(1, 9803); deu_erro := false;
  EXCEPTION WHEN OTHERS THEN deu_erro := true; END;
  PERFORM public.exigir(NOT deu_erro, 'conta ativa de volta: o Inicio abre de novo');
END $$;

-- ---------------------------------------------------------------------------
-- 56. A faixa "Feitas hoje" tambem diz quem fez na tarefa COM DONO
-- ---------------------------------------------------------------------------
...
DO $$ BEGIN RAISE NOTICE '56. "Feitas hoje" diz quem fez tambem na tarefa com dono'; END $$;
