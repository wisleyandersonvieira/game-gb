-- =========================================================================
-- STGame — o aceite e a entrega pelo tablet numa ida só ao banco.
--
-- Como usar: Supabase -> SQL Editor -> New query -> colar TUDO -> Run.
-- Se der erro, NADA é aplicado: me mande a mensagem.
-- Pode rodar duas vezes sem problema.
--
-- ATENÇÃO: aplique tudo o que veio antes (inclusive o som por loja).
-- E aplique ESTE ARQUIVO ANTES de publicar a versão nova: o tablet novo
-- chama funções que só existem depois dele.
--
-- Este arquivo é UMA migração só:
--   20260929130000_pin_do_tablet_numa_ida.sql
--
-- O QUE MUDA PARA QUEM JÁ USA:
--   * Nada de regra: o PIN continua conferido no servidor a cada ação, a
--     trava conta cada tentativa como antes, e o rodízio e as outras regras
--     do aceite e da entrega são as mesmas (as funções novas chamam as
--     antigas por dentro).
--   * A trava de tentativas e a busca do PIN passam a usar índice, em vez de
--     varrer as tabelas.
--   * Três índices novos na tabela de tentativas (é pequena: criar leva
--     segundos).
-- =========================================================================
