-- =========================================================================
-- STGame — a TV passa a ser configurável por loja, e ganha dois blocos.
--
...
--   20260929101100_tv_configuravel.sql
--
-- O QUE MUDA PARA QUEM JÁ USA: nada. As lojas que já existem começam com
-- exatamente o conjunto que a TV mostra hoje (as duas faixas e as colunas
-- "para fazer", "esperando o gestor" e "pódio de hoje"), e os dois blocos
-- novos nascem desmarcados.
-- =========================================================================
...
-- =========================================================================
DO $verifica$
DECLARE v jsonb; v_conta integer; v_loja integer;
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_schema = 'public' AND table_name = 'lojas'
                    AND column_name = 'tvblocos') THEN
    RAISE EXCEPTION 'Faltou a coluna lojas.tvblocos.';
  END IF;

  -- COMPATIBILIDADE: nenhuma loja pode ter saído daqui já configurada.
  IF EXISTS (SELECT 1 FROM public.lojas WHERE tvblocos IS NOT NULL) THEN
    RAISE EXCEPTION 'Alguma loja já saiu configurada. Isto não devia acontecer.';
  END IF;

  -- Os dois blocos que faltavam têm de existir na MESMA consulta da TV.
  SELECT contaid, lojaid INTO v_conta, v_loja FROM public.lojas LIMIT 1;
  IF v_conta IS NOT NULL THEN
    v := public.montar_painel(v_conta, v_loja, true);
    IF NOT (v ? 'emandamento') OR NOT (v ? 'podiomes') THEN
      RAISE EXCEPTION 'O painel não traz os dois blocos novos: a TV teria de fazer outra consulta.';
    END IF;

    -- E a loja sem configuração tem de mostrar o conjunto de antes.
    v := public.tv_blocos_da_loja(v_conta, v_loja);
    IF NOT (v->>'parafazer')::boolean OR NOT (v->>'podiohoje')::boolean THEN
      RAISE EXCEPTION 'A loja sem configuração deixaria de mostrar o que mostrava antes.';
    END IF;
  END IF;

  RAISE NOTICE 'tudo certo: a TV é configurável por loja.';
END $verifica$;
