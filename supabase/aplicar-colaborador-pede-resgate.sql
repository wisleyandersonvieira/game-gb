-- =========================================================================
-- STGame — o colaborador PEDE resgate pelo celular.
--
...
--   20260929101000_colaborador_pede_resgate.sql
--
-- O QUE MUDA PARA QUEM JÁ USA: nada nos resgates que já existem. A coluna nova
-- da origem nasce vazia neles, e vazio quer dizer "registrado pelo gestor".
-- =========================================================================
...
BEGIN
  SELECT array_agg(f) INTO v_falta
    FROM unnest(ARRAY['eu_premios', 'eu_resgates', 'eu_pedir_resgate', 'eu_cancelar_resgate']) f
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                  WHERE table_schema = 'public' AND table_name = 'resgates' AND column_name = 'origem') THEN
    RAISE EXCEPTION 'Faltou a coluna da origem do resgate.';
  END IF;

  -- COMPATIBILIDADE: nenhum resgate antigo pode ter ganhado origem.
  IF EXISTS (SELECT 1 FROM public.resgates
              WHERE origem IS NOT NULL AND datasolicitacao < now() - interval '1 minute') THEN
    RAISE EXCEPTION 'Algum resgate antigo ganhou origem. Isto não devia acontecer.';
  END IF;

  -- Nenhuma eu_* pode estar liberada para o usuário logado.
  ...
  -- A lista do gestor tem de trazer a origem.
  IF (SELECT prosrc FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
       WHERE n.nspname = 'public' AND p.proname = 'listar_trocas') NOT LIKE '%origem%' THEN
    RAISE EXCEPTION 'A lista de resgates do gestor não mostra de onde veio o pedido.';
  END IF;

  IF EXISTS (SELECT 1 FROM public.contas c
              WHERE NOT EXISTS (SELECT 1 FROM public.configuracoes g
                                 WHERE g.contaid = c.contaid AND g.chave = 'MAX_RESGATES_PENDENTES')) THEN
    RAISE EXCEPTION 'O limite de pedidos pendentes não chegou em alguma conta.';
  END IF;

  RAISE NOTICE 'tudo certo: o colaborador pode pedir resgate.';
END $verifica$;
